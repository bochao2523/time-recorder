...
**The Independent Timer Rule.** 每个任务拥有独立的开始时间、暂停状态和结束操作；新增或结束一个任务不得改变其他计时器的累计时间与运行状态。
...
底部主导航固定呈现五个等宽目的地格：今日、阅读、记录、统计与设置。阅读是“选书—计时—结束补页码”的独立操作面；保存后将分钟同步到今日页的阅读大类。未激活项为瓶绿底、浅色文字和低对比黄线分隔；激活项整格切换为铬黄底与瓶绿文字。移动端图标在上、标签在下；640px 起变为横向组合。每格覆盖完整导航高度并保留内嵌焦点环。
...
每个分类是一个 14px 浅色缝制课程，折叠头部至少 4.5rem 高。44px 绿色图标底座、标题、项目数、总分钟和展开动作形成稳定一行；展开内容使用缝合虚线分隔，任务名、分钟、快捷计时和删除控件保持触控安全。历史任务以稳定的横向快捷栏呈现，点击后填入空行并直接聚焦分钟输入，不重复创建同名任务。

### Reading Analysis Course

阅读页用书籍维度替代逐条流水列表：深色课程承载当前页、总页数、进度和累计读数，浅色课程承载日均效率与完读预估；24 小时时段和 14 天打卡使用原生 HTML/CSS 柱形图。没有总页数、页码或精确计时时只显示真实可计算数据，并用明确文案提示如何补齐，不伪造分析结果。
...
- 普通“阅读”大类是一般任务：不要求书名或页码，可使用正计时与倒计时。
- 只有从底部“阅读”页面书架开始的计时才是书籍阅读，结束时强制记录页码。
- 添加书籍必须同时填写书名与总页数；进度、预计完读和已读完状态都由页码记录推导。
- 书籍计时使用全屏专注界面，暂停与结束按钮固定在底部安全区，计时文字使用固定宽度数字，禁止因 tick 产生布局抖动。
